...
print_header "Reviewer Load Distribution"
...
PENDING_FILE=$(mktemp)
trap "rm -f $REQUESTED_FILE $COMPLETED_FILE $RESPONSE_FILE $PENDING_FILE $PR_LIFECYCLE_FILE" EXIT
...
while IFS= read -r pr; do
  ...
  echo "$pr" | jq -c '.timelineItems[] | select(.__typename == "PullRequestReview")' | while IFS= read -r review; do
    ...
    # Find matching review request for response time
  ...
  # Track pending reviews (requested but not yet reviewed in this PR)
...
ALL_REVIEWERS_FILE=$(mktemp)
trap "rm -f $REQUESTED_FILE $COMPLETED_FILE $RESPONSE_FILE $PENDING_FILE $ALL_REVIEWERS_FILE $PR_LIFECYCLE_FILE" EXIT
...
total_completed=0
REVIEWER_COMPLETED_FILE=$(mktemp)
trap "rm -f $REQUESTED_FILE $COMPLETED_FILE $RESPONSE_FILE $PENDING_FILE $ALL_REVIEWERS_FILE $REVIEWER_COMPLETED_FILE $PR_LIFECYCLE_FILE" EXIT
...
if [[ $total_completed -gt 0 && -s "$REVIEWER_COMPLETED_FILE" ]]; then
  ...
  # Show overloaded reviewers (pending > 3)
