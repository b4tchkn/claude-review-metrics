...
TOTAL_FILE=$(mktemp)
SLOWEST_FILE=$(mktemp)
# Review Cycles sub-phase tracking
RC_INSTANT_FILE=$(mktemp)
RC_SECOND_WAIT_FILE=$(mktemp)
RC_OTHER_FILE=$(mktemp)
# Response time by review round
RT_FIRST_FILE=$(mktemp)
RT_REREVIEW_FILE=$(mktemp)
trap "rm -f $WAIT_FILE $REVIEW_FILE $MERGE_FILE $TOTAL_FILE $SLOWEST_FILE $RC_INSTANT_FILE $RC_SECOND_WAIT_FILE $RC_OTHER_FILE $RT_FIRST_FILE $RT_REREVIEW_FILE $PR_LIFECYCLE_FILE" EXIT
...
while IFS= read -r pr; do
  ...
  # Find first review request or first review (whichever came first as "ready for review")
  ...
  if [[ -n "$first_review" ]]; then
    ...
    # Phase 1: Wait for Review (from creation/ready to first review)
    if [[ "$first_review_ts" -ge "$review_ready_ts" ]]; then
      wait_seconds=$((first_review_ts - review_ready_ts))
      echo "$wait_seconds" >> "$WAIT_FILE"
    fi

    # Phase 2: Review Cycles (from first review to last approval)
    if [[ -n "$last_approval" ]]; then
      ...
      if [[ "$last_approval_ts" -ge "$first_review_ts" ]]; then
        ...
        # Sub-phase: classify what caused the review cycle time
      ...
      # Phase 3: Merge Delay (from last approval to merge)
  ...
  # Classify response times by review round (first review vs re-review)
  # Get all review requests sorted by time
  ...
  # For each reviewer, find their request-review pairs
  echo "$pr" | jq -r '[.timelineItems[] | select(.__typename == "ReviewRequestedEvent") | .requestedReviewer.login // empty] | unique | .[]' | while IFS= read -r reviewer; do
    ...
    # Get all requests for this reviewer (sorted)
    req_times=$(echo "$review_requests_json" | jq -r --arg r "$reviewer" '[.[] | select(.reviewer == $r) | .time] | .[]')
    # Get all reviews by this reviewer (sorted)
...
# Calculate avg from a file of values
calc_avg() {
  local file=$1
  if [[ ! -s "$file" ]]; then
    echo "0"
...
if [[ -n "$total_avg_val" && "$total_avg_val" -gt 0 ]]; then
  ...
  # Show Review Cycles breakdown when it's the bottleneck or significant
  rc_total=0
  ...
  if [[ $rc_total -gt 0 ]]; then
    ...
    # Targeted suggestion based on actual cause
...
# Response time comparison: first review vs re-review
